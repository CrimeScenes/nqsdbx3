...
local function predictTargetPosition(targetPlayer, deltaTime)
    ...
    if humanoid then
        ...
        -- Offset hitbox for different ranges
        local offset = Vector3.new(0, 0, 0)
        if distance < 50 then
            offset = Vector3.new(0, 2, 0)  -- Close range: adjust slightly above the center
        elseif distance < 200 then
            offset = Vector3.new(0, 1, 0)  -- Mid range: adjust moderately
        else
            offset = Vector3.new(0, 0, 0)  -- Far range: aim at the center
        end

        -- Predict future position and apply offset
...
-- Function to aim at the target's hitbox
