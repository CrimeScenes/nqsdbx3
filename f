...
-- Getting the configuration from shared.Forbidden
local config = shared.Forbidden.TriggerBot.Settings
local resetTime = config.ResetTime
local keybind = config.Keybind
local enableLegitMode = config.Preds.EnableLegitMode
local ignoreKnife = config.Preds.Safety.IgnoreKnife
...
-- Function to predict target's hitbox position based on their movement
local function predictTargetPosition(targetPlayer, deltaTime)
    ...
    if humanoid then
        ...
        -- Adjust prediction factor based on distance
        if distance <= 25 then
            predictionFactor = 0.08  -- Close range: more accurate prediction
        elseif distance <= 41 then
            predictionFactor = 0.15  -- Mid range: moderate prediction
        else
            predictionFactor = 0.2  -- Far range: less accurate prediction
        end

        -- Offset hitbox for different ranges (smaller offsets for closer distances)
        local offset = Vector3.new(0, 0, 0)
        if distance <= 25 then
            offset = Vector3.new(0, 0.4, 0)  -- Close range: smaller hitbox offset
        elseif distance <= 41 then
            offset = Vector3.new(0, 0.7, 0)  -- Mid range: smaller hitbox offset
        else
            offset = Vector3.new(0, 1, 0)  -- Far range: slightly larger hitbox offset
        end

        -- Predict future position and apply offset (including air prediction)
        local prediction = targetPlayer.Character.HumanoidRootPart.Position + velocity * deltaTime * predictionFactor + offset

        -- Air prediction: if the player is in the air (jumping)
        if humanoid:GetState() == Enum.HumanoidStateType.Physics or humanoid:GetState() == Enum.HumanoidStateType.Jumping then
            -- Apply gravity influence and velocity for jumping prediction
            local gravity = game:GetService("Workspace").Gravity
            local predictedY = prediction.Y + (velocity.Y * deltaTime) - (gravity * deltaTime * deltaTime)
            prediction = Vector3.new(prediction.X, predictedY, prediction.Z)
        end

        -- Store last few positions for smoothing
...
-- Function to aim at any part of the target's body
local function aimAtTargetHitbox(targetPlayer)
    ...
    for _, partName in pairs(AllBodyParts) do
        ...
        if part then
            local predictedPos = predictTargetPosition(targetPlayer, 0.1)  -- Predict the future position of the target
            local screenPos, onScreen = Camera:WorldToViewportPoint(predictedPos)
            if onScreen and isWithinViewEra(predictedPos) then
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                aimAtTargetHitbox(TargetPlayer)
            end
        end
    end
end
